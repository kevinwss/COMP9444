...
def init(env, env_name):
    ...
    if (env_name == "Pendulum-v0"):
        global action_map
        Pendulum_action_dim = 50 # User define dimensions
        iscontinuous = True
        action_dim = Pendulum_action_dim+1

        action_map = dict()
        high = env.action_space.high[0]
        low = env.action_space.low[0]
        for i in range(Pendulum_action_dim):
            action_map[i] = low+(high-low)/Pendulum_action_dim*i
        action_map[i+1] = high
...
def get_env_action(action):
    ...
    if iscontinuous==True:
        #print("env_action",action_map[action])
...
def qtrain(env, state_dim, action_dim,
    ...
    for episode in range(num_episodes):
        ...
        for step in range(ep_max_steps):
            ...
            next_state, reward, done, _ = env.step(env_action)#reward
            
            reward_hidden = reward

            if (sys.argv[1] == 'CartPole-v0' or sys.argv[1] == ''):
                x, x_dot, theta, theta_dot = next_state   #rivise reward 120
                r1 = (2.4 - abs(x))/2.4 - 0.8
                r2 = (0.2618 - abs(theta))/0.2618 - 0.5
                reward_hidden = r1+r2
                

            if (sys.argv[1] == 'MountainCar-v0'): #Modify the reward episode 60 

                position, velocity = next_state
                #reward_hidden =abs(velocity)*100
                #print(reward_hidden)
                if (reward + next_state[1]>-1):
                    reward_hidden =reward+ next_state[1]*100
            ...
            update_replay_buffer(replay_buffer, state, action, reward_hidden,
