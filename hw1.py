...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    batch_xentropy = -tf.reduce_sum(Y*tf.log(preds+1e-10) , reduction_indices=[1])
    batch_loss = tf.reduce_mean(batch_xentropy)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    w = weight_variable([24*24*outputsize,10])
    b = bias_variable([10])
    
    
    conv1=tf.layers.conv2d(inputs=X,filters=convlayer_sizes[0],kernel_size=filter_shape,activation=tf.nn.relu)
    conv2=tf.layers.conv2d(inputs=conv1,filters=convlayer_sizes[1],kernel_size=filter_shape,activation=tf.nn.relu)

    conv2_flat = tf.reshape(conv2, [-1, 24*24*outputsize])
